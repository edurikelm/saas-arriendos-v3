...
**Series de ingresos (ADR-0030):** la serie mensual `cash.byMonth` y el desglose por método `cash.byMethod` del rango en `ReportDecisionSummary` se calculan con el mismo predicate de cash basis (`sum(cash.byMethod) === collectedCash`). La serie anual ya no forma parte del resumen (ADR-0035); `buildAnnualCollectedCash` sigue en `revenue-series.ts` para `getYearlySummary`. `paymentCount` es el conteo de pagos eligible (no `reservationCount`). La reconciliación garantiza `totalCash === sum(byMonth.collectedCash) === sum(byMethod)`.
