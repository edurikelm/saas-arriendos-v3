...
- **Micro-label** (`text-[9px]`): Un segundo escalón, un paso debajo del 10px whisper, para metadata densa dentro de tablas y cards donde el 10px ya compite con el dato principal de la fila (ej. el badge de duración junto al nombre del cliente en `dashboard/page.tsx`, el sublabel "N noches" bajo el nombre en `OccupancyStrip`, los toggles Diario/Mensual de `reservation-form.tsx`, el label de canal en `property-card.tsx`, y `ReservationPill`). Su forma habitual es `font-bold uppercase tracking-tight` (mismo espíritu que el whisper de 10px, un paso más chico). **No todo `text-[9px]` lleva esa forma:** dos usos en `dashboard/page.tsx` (el sublabel de estado bajo un `ReservationPill` y el sufijo "/mes" bajo un monto mensual) van planos, sin `uppercase` ni `font-bold`, porque son sufijos de unidad y sublabels de continuación donde el uppercase leería mal. El escalón se define por **tamaño** (9px), no por forma — la forma con `uppercase`/`bold` es la doctrina del 10px whisper aplicada un paso más abajo, pero no es requisito para calificar como micro-label.
