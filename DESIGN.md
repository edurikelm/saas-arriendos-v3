...
**The Fill-vs-Text Rule.** Cada tono semántico tiene DOS colores, y no son intercambiables: el de **relleno** (`--success`, `--warning`, `--info`, `--destructive`) para fondos y barras, y el **legible sobre card** (`--success-text`, `--warning-text`, `--info-text`, `--destructive-text`) para texto e íconos. Usar el de relleno como color de texto es el error que produjo #235: `text-destructive` sobre card mide **3.76:1 en claro**, bajo el mínimo AA de 4.5:1, y la excepción de "texto grande" no aplica porque el producto usa 10-12px casi en todas partes. Valores medidos de `--destructive-text`: **5.34:1 claro / 5.46:1 oscuro** sobre card, y **4.75:1 / 4.90:1** sobre `bg-destructive/10`, que es el combo real del producto y por eso también entra en el criterio.

Son **tres** niveles, no dos, y confundir los dos últimos es el error sutil:

| Nivel | Tokens | Para qué |
|---|---|---|
| Relleno | `--{tono}` | Fondos, barras, puntos de estado |
| Sobre el relleno | `--{tono}-foreground` | El texto que va ENCIMA de una superficie rellena (texto del badge) |
...
- **Micro-label** (`text-[9px]`): Un segundo escalón, un paso debajo del 10px whisper, para metadata densa dentro de tablas y cards donde el 10px ya compite con el dato principal de la fila (ej. el chip "Llega"/"Sale" de `dashboard-agenda.tsx`, los toggles Diario/Mensual de `reservation-form.tsx`, los labels de stats — Unidades/Precio Noche/Precio Mes — en `property-card.tsx`, y `ReservationPill`). Su forma es `font-bold uppercase tracking-tight` (mismo espíritu que el whisper de 10px, un paso más chico) — hoy sin excepciones planas en el código (`grep -rn "text-\[9px\]" src`). El escalón se define por **tamaño** (9px), no por forma: si en el futuro aparece un sufijo de unidad o un sublabel de continuación donde el uppercase leería mal, puede ir plano sin dejar de calificar como micro-label.
...
- **Value:** `text-xl font-bold tabular-nums`. Color: `text-{tone}-text` cuando `tone` es `success`, `warning` o `destructive`; `default` e `info` quedan en `text-foreground` (estados neutros sin énfasis de color). **No** `text-{tone}` a secas: son tokens de RELLENO y sobre card miden 3.03:1 (`success`) y 2.05:1 (`warning`) en claro, bajo AA. Ver The Fill-vs-Text Rule, que ya documenta la medición de este mismo valor a 20px bold.
- **Icon container (opcional):** `size-9 rounded-xl bg-{tone}/10 text-{tone}-text` en esquina superior derecha; `bg-muted text-muted-foreground` para `default`. Sin container si no se pasa icon. El ícono va sobre el tinte al 10%, no sobre card, y ahí los tokens de relleno miden 2.59 / 2.55 / 1.81:1 en claro — bajo el piso de 3:1 que WCAG 1.4.11 pide a un gráfico no textual.
- **Indicator (opcional):** `text-[10px] font-medium mt-1` con TrendingUp (positive) o AlertTriangle (warning/neutral). Usa `text-success-text` / `text-warning-text`, igual que el value — es texto sobre `--card`, no sobre el relleno del mismo tono, así que el nivel que aplica es `-text`, sin excepción por tamaño. **Nunca `text-destructive-foreground`**: ese rompe el patrón por ser blanco — `--destructive` hace doble uso como relleno de botón destructivo — y sobre card daría 1.00:1.
...
**The Row Isolation Rule.** Las filas de `<DataTable>` no llevan `onClick` que navegue. Solo el botón o link de la columna de acciones navega al detalle. Previene selección accidental de texto, doble-trigger y problemas de accesibilidad. **Corolario:** como la fila no es clickeable, esa columna es el *único* camino a las acciones — si se sale del área visible por ancho, la tabla queda de solo lectura y nada lo señala.

**The One KPI Voice Rule.** Existe un solo primitive de KPI: `<KpiCard>`. Si necesitas algo "diferente", es señal de que el KPI no calza — replantéalo, no inventes un primitive paralelo.
...
- **Do** usar `<ReservationPill>` solo para el estado temporal de una reserva (tabla de reservas, detalle de reserva). Para badges de estado en otros contextos — pagos, cobranza, propiedades — usar `<Badge variant>`. En una lista agrupada por estado, ninguno de los dos: el color va en el encabezado del grupo (ADR-0033).
...
- **Don't** usar `bg-amber-50 border-amber-200`, `bg-green-50 border-green-200` ni similares para alert-boxes informativas. Usar `bg-warning/10 border-warning/20 text-warning-text` (o el tone que corresponda) — texto sobre `--card`, así que va el nivel `-text`, no `-foreground`. Las 3 instancias que existían en `billing-client.tsx` y `cancel-subscription-dialog.tsx` ya están migradas.
...
- **Don't** usar `text-destructive`, `text-success`, `text-warning` ni `text-info` como color de texto o de ícono. Son tokens de RELLENO: sobre card miden 3.76 / 3.03 / 2.05 / 2.97:1 en claro, todos bajo AA. Va el `-text` de cada tono (ver The Fill-vs-Text Rule).
- **Don't** usar `text-{tono}-foreground` para una cifra, un titular coloreado, o cualquier texto/ícono sobre `--card`. `bg-{tono}` + `text-{tono}-foreground` **no** es "el par del badge relleno siempre correcto" — eso era la creencia hasta que se midió: en success/warning/info ese par medía **1.24-1.67:1 en oscuro**, invisible sobre su propio relleno (el relleno casi no cambia entre temas, pero `-foreground` se invertía L=0.30↔L=0.85). Se corrigió fijando `--success/warning/info-foreground` a un único valor sin tema — igual que `--destructive-foreground`, que nunca tuvo el problema porque es blanco fijo. Con el fix, el par sobre relleno pasa AA en los dos temas (4.56-4.63:1 claro / 4.63-5.63:1 oscuro) y sigue siendo el nivel correcto para texto ENCIMA del relleno opaco (badge). Pero para texto sobre `--card` — cifra de KPI, titular, indicator de 10px — sigue sin servir: en claro se lee negro y el tono deja de comunicar. Ahí va `-text`, sin excepción por tamaño de fuente.
...
- **Don't** introducir tamaños de fuente arbitrarios (`text-[Npx]`, `text-[N.Nrem]`) fuera de los dos escalones documentados, `text-[10px]` (The 10px Whisper Rule) y `text-[9px]` (micro-label, ver Typography → Hierarchy). Cualquier otro tamaño va en la escala de Tailwind (`text-xs`, `text-sm`, `text-base`...). Limpieza de 2026-09-10: se eliminaron cuatro tamaños sin base — `text-[11px]` (10 usos, sin patrón propio y a un solo píxel de su vecino `text-xs`, así que el ojo no distinguía la intención), `text-[13.5px]` (2 usos, un tamaño fraccionario que no existe en ninguna escala), y `text-[8px]` (1 uso, bajo el piso del micro-label de 9px). La única excepción viva es `text-[0.8rem]` en `src/components/ui/button.tsx` y `src/components/ui/calendar.tsx`, heredada de shadcn upstream — cambiarla altera el tamaño de los botones `sm` y de las celdas del calendario en todo el producto, una decisión aparte de esta limpieza.
