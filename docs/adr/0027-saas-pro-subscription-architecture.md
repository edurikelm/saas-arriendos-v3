...
**Preapprovals huérfanos previos a este fix:** esta protección previene casos nuevos hacia adelante, pero no reconcilia retroactivamente preapprovals que hayan quedado vivos por una `adminCancelSubscription` anterior a este cambio. Si hiciera falta esa reconciliación en algún momento: buscar en el panel de MP por `payer_email` o por el prefijo de `external_reference` (`{userId}:`, ver §1), y revisar los eventos `payment_unapplied` y los warnings "Subscription not found" del webhook (`/api/webhooks/mercadopago-pro`) como señal de cobros que ya no tenían dónde aplicarse. Consulta hecha el 2026-09-24 sobre la base de producción: 2 filas `Subscription` (1 `AUTHORIZED`, 1 `PENDING`), ninguna con `mpPreapprovalId`, y ningún `SubscriptionEvent` de origen webhook. Es una señal de que no hubo cobros reales, pero no una prueba: las filas reemplazadas se borran y un `subscription.update` fallido después de `createPreapproval` deja un preapproval sin fila, y ninguno de los dos casos aparece en una consulta local. La prueba real es la búsqueda del lado de MP descrita arriba.
