...
**Cancelación de owner y de admin cancelan el mismo preapproval (evita doble cobro):** tanto `cancelMySubscription` como `adminCancelSubscription` (`/admin/users/[id]`) cancelan primero el preapproval en Mercado Pago y solo después aplican el evento de lifecycle — si la llamada a MP falla, ninguno de los dos toca el estado local ni escribe auditoría (`AdminActionLog` en el caso admin), y el operador puede reintentar. Antes de este fix, `adminCancelSubscription` marcaba `CANCELLED` localmente sin cancelar en MP, así que la cuenta central de RentalPro seguía cobrando al owner cada mes hasta que alguien lo notara manualmente. El evento `admin_cancel` y el `AdminActionLog` correspondiente registran `mpCancelled: true/false` (falso solo cuando la fila nunca tuvo `mpPreapprovalId`) para que la auditoría distinga "cancelado en MP" de "no había nada que cancelar en MP".

**`startProUpgrade` también verifica el preapproval anterior antes de reemplazar la fila:** una fila EXPIRED/FAILED/CANCELLED-expirada puede seguir teniendo un preapproval vivo en MP si el cron `EXPIRED_CHECK` la marcó localmente sin que llegara el webhook de renovación. `ensurePreviousPreapprovalStopped` (en el mismo archivo) consulta `fetchPreapproval` y cancela si el estado en MP no es `"cancelled"`, ANTES de la transacción de reemplazo — no dentro, porque una transacción de DB no debe hacer I/O de red. Si la consulta o la cancelación fallan, no se borra la fila vieja ni se crea un preapproval nuevo. Con esto nunca hay más de un preapproval vivo por owner, cerrando el mismo hueco de doble cobro que las dos cancelaciones de arriba.
